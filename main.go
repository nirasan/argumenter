...
func WriteFile(p packageDecl, types []string, overwrite bool) {
	filename := p.Dir + "/" + strings.TrimRight(p.File, ".go") + "_argumenter.go"
	exist := fileIsExist(filename)

	var f *os.File
	var e error
	if !exist || overwrite {
		f, e = os.OpenFile(filename, os.O_CREATE|os.O_RDWR, 644)
	} else {
		f, e = os.OpenFile(filename, os.O_APPEND, 644)
	}
	if e != nil {
		panic(e)
	}
	...
	if !exist || overwrite {
		...
		import "errors"
		`))
		e := t.Execute(w, struct{ Name, Types string }{p.Name, strings.Join(types, ",")})
		if e != nil {
			panic(e)
		}
	}
	for _, s := range p.SelectStructs(types) {
		e := s.Generate(w)
		if e != nil {
			panic(e)
		}
